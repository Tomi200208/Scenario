...
def _callback(
    ...
    body = {
        ...
        'runId': run_id,
        'status': status,
        'progress': 100 if status in ('completed', 'failed', 'cancelled') else 50,
        'currentStage': 'Simulation completed' if status == 'completed' else 'Simulation failed',
        'result': result or {},
    }
    if error:
        body['error'] = error
        body['result'] = {**(result or {}), 'error': error}
    if worker_run_id:
        body['workerRunId'] = worker_run_id
    if report_markdown:
        body['reportMarkdown'] = report_markdown

    callback_url = cb.get('url')
    callback_token = cb.get('token')
    if not callback_url or not callback_token:
        logger.error("ScenarioPilot callback URL or token is missing")
        return False
    ...
    for attempt in range(1, 4):
        try:
            ...
            logger.warning("ScenarioPilot callback attempt %s/3 failed: %s", attempt, exc)
            if attempt < 3:
...
def _run_simulation(sp_run_id: str, project_id: str, seed: dict, cb: dict):
    ...
    try:
        ...
        simulation_requirement = (
            (seed or {}).get('simulationRequirement')
            or (seed or {}).get('simulation_requirement')
            or f"Simulate stakeholder responses, interactions, emerging narratives, and strategic implications for: {theme}"
        )

        # 1) Ontology + project from the seed (multipart file upload).
        files = {'files': ('scenariopilot_seed.txt', io.BytesIO(seed_text.encode('utf-8')), 'text/plain')}
        data = {
            'project_name': f"SP: {theme}"[:80],
            'simulation_requirement': simulation_requirement,
            'additional_context': 'ScenarioPilot foresight seed bundle.',
        }
        ...
        # 2) Build the graph, then poll the build task.
        build = _post('/api/graph/build', json={'project_id': mf_project})
        task_id = (build.get('data') or build).get('task_id')
        build_state = {}
        if task_id:
            build_state = _poll(
                lambda: _get(f"/api/graph/task/{task_id}"),
                lambda p: str((p.get('data') or p).get('status', '')).lower() in ('completed', 'success', 'done'),
            )
        graph_id = (build.get('data') or build).get('graph_id')
        build_task = build_state.get('data', build_state) if isinstance(build_state, dict) else {}
        build_result = build_task.get('result') or {}
        graph_id = graph_id or build_result.get('graph_id')
        ...
        # 4) Prepare entities (LLM profiles), poll prepare/status.
        ...
        # 6) Generate the report, poll its task, then fetch its content.
        rep = _post('/api/report/generate', json={'simulation_id': simulation_id})
        report_id = (rep.get('data') or rep).get('report_id')
        report_task = (rep.get('data') or rep).get('task_id')
        report = {}
        if report_task:
            report_state = _poll(
                lambda: _post(
                    '/api/report/generate/status',
                    json={'task_id': report_task, 'simulation_id': simulation_id},
                ),
                lambda p: str((p.get('data') or p).get('status', '')).lower() in ('completed', 'success', 'done'),
            )
            report_data = report_state.get('data', report_state) if isinstance(report_state, dict) else {}
            report_id = report_id or report_data.get('report_id')
        if report_id:
        ...
        _callback(
            ...
            worker_run_id=simulation_id,
            report_markdown=report.get('markdown_content') if isinstance(report, dict) else None,
        )
